...
异常缓存策略 SHALL 只评估被装饰业务方法实际抛出或拒绝的值。Provider 解析、Provider 读取、缓存 key 解析降级、错误策略求值、错误编解码及 Logger 故障 MUST NOT 被当作业务异常写入缓存。

#### Scenario: Provider 解析或读取失败

- **WHEN** Provider 不存在，或 Provider 读取因 Redis 不可用、连接超时或其他基础设施原因拒绝
- **THEN** 原 Provider 错误按既有路径传播，不调用异常筛选器或错误 codec
- **AND** 不执行被装饰业务方法、不写入异常条目、不切换其他 Provider
...
- **AND** 当前缓存结果、业务结果或原始 Provider 错误保持不变
