...
describe('装饰器元数据存储和读取', () => {
  describe('@HttpClient', () => {
    it('should store config in metadata', () => {
      ...
      const config = (Reflect as unknown as { getMetadata: (key: symbol, target: object) => unknown }).getMetadata(
        Symbol.for('bics:http-client:config'),
        TestService
      );
    ...
    it('should store config with middlewares', () => {
      ...
      const config = (Reflect as unknown as { getMetadata: (key: symbol, target: object) => unknown }).getMetadata(
        Symbol.for('bics:http-client:config'),
        TestService
      ) as { baseURL: string; middlewares: unknown[] };
