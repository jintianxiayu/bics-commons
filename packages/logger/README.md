...
| `masking.fields`  | `SensitiveFieldConfig` | `{}`   | 字符串新增或覆盖掩码模板；`false` 取消指定字段的策略；字段匹配不区分大小写 |

`SensitiveFieldConfig` 的定义为 `Record<string, string | false>`。
...
当业务字段与内置字段同名、但确定不承载敏感信息时，可以显式取消该字段的脱敏：
...
`false` 必须是布尔值且不能加引号；`'false'` 是合法的字符串模板，会把匹配值替换为文本 `false`。
退出规则按完整字段名大小写不敏感地匹配，因此 `password: false` 同时匹配 `Password`，但不会连带取消
`passwd`、`pwd` 等其他内置别名，也不会按前缀匹配 `passwordHash`。

字段策略是应用级配置，对所有命名 Logger、Console/File 输出和任意嵌套层级生效。如果退出字段的值是对象，
该对象仍会递归复制并检查内部字段；例如外层 `password` 退出后，内部 `token` 仍按内置策略脱敏。

包含字段值 `false` 的配置需要支持该能力的 Logger 版本。旧版会在初始化阶段将其作为非字符串模板拒绝，
因此部署时应先升级所有读取该配置的 Logger 实例，再启用字段退出；回滚旧版前应先移除这些布尔值。

> 脱敏只处理元数据字段，不解析 `message` 文本。不要把令牌、密码或个人信息拼接进日志消息。
> 显式退出可能使原始值进入全部日志输出；仅应在确认该字段名不承载敏感信息，并评估所有同名字段来源后使用。
