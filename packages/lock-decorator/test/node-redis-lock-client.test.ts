...
import { NodeRedisLockClientSource } from '../src/core/redis-lock-client';
import { RedisLockProvider } from '../src/core/redis-lock-provider';

const source = {
    set: jest.fn<Promise<unknown>, Parameters<NodeRedisLockClientSource['set']>>(),
    eval: jest.fn<Promise<unknown>, Parameters<NodeRedisLockClientSource['eval']>>(),
};
const client = createNodeRedisLockClient(source);
const request = Object.freeze({ key: 'order:123', value: 'token', ttlMs: 1234 });

beforeEach(() => jest.resetAllMocks());

it('redis-lock-client/A04 node-redis 条件写入成功', async () => {
    source.set.mockResolvedValue('OK');
    await expect(client.setIfAbsent(request)).resolves.toBe(true);
    expect(source.set).toHaveBeenCalledWith(request.key, request.value, { PX: request.ttlMs, NX: true });
});

it('redis-lock-client/A05 node-redis 条件写入发生竞争', async () => {
...
it.each([
    ...
    { keys: [], arguments: [] },
])('redis-lock-client/A06 node-redis 脚本参数与响应保持一致：%j', async (options) => {
    ...
    expect(source.eval).toHaveBeenCalledWith('return {KEYS, ARGV}', {
        keys: options.keys,
...
it('redis-lock-client/A07 调用上下文与请求保持不变', async () => {
    source.set.mockImplementation(function (this: NodeRedisLockClientSource) {
        expect(this).toBe(source);
        return Promise.resolve('OK');
    });
    source.eval.mockImplementation(function (this: NodeRedisLockClientSource) {
        expect(this).toBe(source);
...
it.each(['', '订单:lock'])('redis-lock-client/L09 key 和 TTL 边界不被适配层改写：%s', async (key) => {
    ...
    for (const ttlMs of [1, 2147483648]) {
        ...
        expect(source.set).toHaveBeenLastCalledWith(key, 'token', { PX: ttlMs, NX: true });
    }
});
